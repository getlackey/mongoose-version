...
module.exports = function(schema, options) {
            ...
            schema.statics.VersionedModel.find({
                ...
                .exec(function(err, docs) {
                    if (docs.length !== 0 && docs[0].refVersion > (self._doc[schema.options.versionKey] || 0)) {
                        var err = new VersionError(self, self._doc[schema.options.versionKey], ['.refVersion']);
                        debug(err);
                        return next(err);
                    }
    ...
    schema.pre('remove', function(next) {
        ...
        schema.statics.VersionedModel.deleteOne({ refId: this._id }, function (err) {
            if (err) {
                debug(err ? err : 'Removed versioned model from mongodb');
            }
